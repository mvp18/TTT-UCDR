...
class Trainer:
	...
	def __init__(self, args):
		...
		self.rotnet_loss = torch.nn.CrossEntropyLoss()
		
		self.RG = np.random.default_rng()

		if args.optimizer=='sgd':
			self.optimizer = optim.SGD(list(self.model.parameters())+list(self.projector.parameters()), weight_decay=args.l2_reg, momentum=args.momentum, nesterov=False, lr=args.lr)
		elif args.optimizer=='adam':
			self.optimizer = optim.Adam(list(self.model.parameters())+list(self.projector.parameters()), eps=1e-8, weight_decay=args.l2_reg)
		...
		self.suffix = '_mixlevel-'+args.mixup_level+'_wcce-'+str(args.wcce)+'_wratio-'+str(args.wratio)+'_wrot-'+str(args.wrot)+'_wmse-'+str(args.wmse)+\
	...
	def get_rotations(self, im_batch, y_a, y_b, ratios, ratio_vec_gt):

		rotated_imgs = []
		rotation_labels = []
		#print('y row - ', y_a.size())
		rot_y_a = torch.zeros((4*y_a.size()[0], 1))
		rot_y_b = torch.zeros((4*y_b.size()[0], 1))
		rot_ratios = torch.zeros((4*ratios.size()[0], 1))
		rot_ratio_vec_gt = torch.zeros([4*ratio_vec_gt.size()[0], len(self.tr_classes)])

		def rotate(img, degree):
			img = img.permute(1,2,0)
			#print(img.size())
			if degree==90:
				img = torch.flip(img.permute(1,0,2), dims=[0])
				#print('90 - ', img.size())
			elif degree==180:
				img = torch.flip(img, dims=[0,1])
				#print('180 - ', img.size())
			if degree==270:
				img = torch.flip(img.permute(1,0,2), dims=[1])
				#print('270 - ', img.size())
		...
		for i, img in enumerate(im_batch):
			...
			all_rots = [img, rotate(img,  90), rotate(img, 180), rotate(img, 270)]
			
			if torch.is_tensor(rotated_imgs):
				rotated_imgs = torch.cat((rotated_imgs, torch.stack(all_rots, dim=0)), dim=0)
				rotation_labels = torch.cat((rotation_labels, torch.LongTensor([0, 1, 2, 3])), dim=0)
			else:
				rotated_imgs = torch.stack(all_rots, dim=0)
				rotation_labels  = torch.LongTensor([0, 1, 2, 3])

			#print('y row - ', ratios.size())
			#print('y row - ', ratio_vec_gt[i, :].size())
			rot_y_a[4*i:4*i+4] = torch.stack(4*[y_a[i].view(1)])
			rot_y_b[4*i:4*i+4] = torch.stack(4*[y_b[i].view(1)])
			rot_ratios[4*i:4*i+4] = torch.stack(4*[ratios[i].view(1)])
			rot_ratio_vec_gt[4*i:4*i+4, :] = torch.stack(4*[ratio_vec_gt[i, :]])


		indices = torch.randperm(rotated_imgs.size()[0])
		rotated_imgs=rotated_imgs[indices].cuda()
		rotation_labels=rotation_labels[indices].cuda()
		rot_y_a = rot_y_a.squeeze()[indices].long().cuda()
		rot_y_b = rot_y_b.squeeze()[indices].long().cuda()
		rot_ratios = rot_ratios.squeeze()[indices].cuda()
		rot_ratio_vec_gt = rot_ratio_vec_gt[indices].cuda()

		return rotated_imgs, rotation_labels, rot_y_a, rot_y_b, rot_ratios, rot_ratio_vec_gt	
	...
	def do_epoch(self):
		...
		for i, (im, cl, domain_ids) in enumerate(self.train_loader):
			...
			rot_im, angles, rot_y_a, rot_y_b, rot_ratios, rot_ratio_vec_gt = self.get_rotations(im, y_a, y_b, ratios, ratio_vec_gt)
			
			if self.args.use_rotated:
				ratio_vec_pred, features = self.model(rot_im)

				if self.args.mixup_level=='feat':
					features, rot_y_a, rot_y_b, rot_ratios, rot_ratio_vec_gt = self.get_mixed_samples(features, cls_numeric, domain_ids, 'feat')

				sem_out = self.model.base_model.last_linear(features)
				pred_angles = self.projector(sem_out)
			
				# Optimize parameters
				cce_l = self.retrieval_loss(sem_out, rot_y_a, rot_y_b, rot_ratios)
				mse_l = self.embedding_loss(sem_out, rot_y_a, rot_y_b, rot_ratios)
				rat_l = self.soft_cce(ratio_vec_pred, rot_ratio_vec_gt)
				rot_l = self.rotnet_loss(pred_angles, angles)

			else:
				ratio_vec_pred, features = self.model(im)
				_, rot_features = self.model(rot_im)

				if self.args.mixup_level=='feat':
					features, y_a, y_b, ratios, ratio_vec_gt = self.get_mixed_samples(features, cls_numeric, domain_ids, 'feat')
			
				sem_out = self.model.base_model.last_linear(features)
				pred_angles = self.projector(self.model.base_model.last_linear(rot_features))
			...
			loss = self.args.wcce*cce_l + self.args.wmse*mse_l + self.args.wratio*rat_l + self.args.wrot*rot_l
			loss.backward()
			...
			rotation_loss.update(rot_l.item(), im.size(0))
			...
			if (i + 1) % self.args.log_interval == 0:
				print('[Train] Epoch: [{0}/{1}][{2}/{3}]\t'
					  ...
					  'rot {rot.val:.4f} ({rot.avg:.4f})\t'
					  'net {net.val:.4f} ({net.avg:.4f})\t'
					  .format(self.current_epoch+1, self.args.epochs, i+1, len(self.train_loader), batch_time=batch_time, 
							  cce=dist_cce_loss, mse=emb_mse_loss, rat=ratio_loss, rot=rotation_loss, net=total_loss))

			# if (i+1)==50:
			#     break

		return {'dist_cce':dist_cce_loss.avg, 'emb_mse':emb_mse_loss.avg, 'ratio_cce':ratio_loss.avg, 'rotation_ce':rotation_loss.avg, 'net':total_loss.avg}
	...
	def do_training(self):
		...
		for self.current_epoch in range(self.start_epoch, self.args.epochs):
			...
			self.logger.add_scalar('Train/Rotation CE Loss', loss['rotation_ce'], self.current_epoch)
