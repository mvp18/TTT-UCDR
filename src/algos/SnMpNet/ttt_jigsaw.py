...
from algos.Jigsaw.jigsaw import jigsaw_ttt
...
def main(args):
	...
	path_log = os.path.join(args.save_path, 'jig'+str(args.jigsaw_version)+'_results', args.dataset, save_folder_name)
	...
	if os.path.isfile(best_model_file):
		...
		path_cp_ttt = os.path.join(args.save_path, 'jig'+str(args.jigsaw_version)+'_models', args.dataset, save_folder_name)

		model_ttt = jigsaw_ttt(ttt_loader, model, 31, args)
		model_save_name = best_model_name[:-len('.pth')] + '_jig-lrc-'+str(args.lr_clf) + '_lrb-'+str(args.lr_net) +\
	...
	if args.dataset=='DomainNet':
		...
		te_loader_query = DataLoader(dataset=data_te_query, batch_size=64*5, shuffle=False, num_workers=args.num_workers, pin_memory=True)
		te_loader_gallery = DataLoader(dataset=data_te_gallery, batch_size=64*5, shuffle=False, num_workers=args.num_workers, pin_memory=True)
