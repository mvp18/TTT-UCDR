...
def main(args):
	...
	path_log = os.path.join(args.save_path, 'jig'+str(args.jigsaw_version)+'_results', 'cross', 'DomainNet', save_folder_name)
	...
	if os.path.isfile(best_model_file):
		...
		path_cp_ttt = os.path.join(args.save_path, 'jig'+str(args.jigsaw_version)+'_models', 'cross', 'DomainNet', save_folder_name)

		model_ttt = jigsaw_ttt(ttt_loader, model, 31, args)
		model_save_name = best_model_name[:-len('.pth')] + '_jig-lr-'+str(args.lr_net)+'_bs-'+str(args.batch_size)+\
						  '_e-'+str(args.epochs)
