...
from algos.Rotnet.rotnet import Rotnet_Online
...
def main(args):
	...
	path_log = os.path.join(args.save_path, 'rot-'+str(args.online)+str(args.rotnet_version)+'_results', args.dataset, save_folder_name)
	...
	if os.path.isfile(best_model_file):
		...
		rotnet_online = Rotnet_Online(splits_query, splits_gallery, model, checkpoint, image_transforms, args, device)

		path_cp_ttt = os.path.join(args.save_path, 'rot-'+str(args.online)+str(args.rotnet_version)+'_models', args.dataset, save_folder_name)
		...
		rotnet_online.train()
		rotnet_online.save_model(path_cp_ttt, model_save_name)	
	else:
	...
	if args.dataset=='DomainNet':
		...
		eval_data = compute_retrieval_metrics(rotnet_online.acc_sk_em, rotnet_online.acc_cls_sk, rotnet_online.acc_im_em, rotnet_online.acc_cls_im)
		...
		eval_data_gzs = compute_retrieval_metrics(rotnet_online.acc_sk_em, rotnet_online.acc_cls_sk, rotnet_online.acc_im_em_gzs, rotnet_online.acc_cls_im_gzs)
