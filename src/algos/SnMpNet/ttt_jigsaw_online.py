...
from algos.Jigsaw.jigsaw import Jigsaw_Online
...
def main(args):
	...
	path_log = os.path.join(args.save_path, 'jig-'+str(args.online)+str(args.jigsaw_version)+'_results', args.dataset, save_folder_name)
	...
	if os.path.isfile(best_model_file):
		...
		jigsaw_online = Jigsaw_Online(splits_query, splits_gallery, model, checkpoint, image_transforms, args, device)

		path_cp_ttt = os.path.join(args.save_path, 'jig-'+str(args.online)+str(args.jigsaw_version)+'_models', args.dataset, save_folder_name)
		...
		jigsaw_online.train()
		jigsaw_online.save_model(path_cp_ttt, model_save_name)	
	else:
	...
	if args.dataset=='DomainNet':
		...
		eval_data = compute_retrieval_metrics(jigsaw_online.acc_sk_em, jigsaw_online.acc_cls_sk, jigsaw_online.acc_im_em, jigsaw_online.acc_cls_im)
		...
		eval_data_gzs = compute_retrieval_metrics(jigsaw_online.acc_sk_em, jigsaw_online.acc_cls_sk, jigsaw_online.acc_im_em_gzs, jigsaw_online.acc_cls_im_gzs)
